...
if(isset($_GET['id'])){
    ...
    if (isset($_POST['btn-update'])) {
        ...
        if ($listObat->updateObat($id, $_POST) > 0) {
            echo "<script>
    ...
    if($id > 0){
        $listObat->getObatById($id);
        $row = $listObat->getResult();

        $data .= 
        '<br><form method="post">
        <div class="form-group">
        <label for="inputNamaObat">Nama Obat</label>
        <input type="text" class="form-control" id="inputNamaObat" placeholder="Masukkan Nama Obat" name="nama_obat" value="'. $row['nama_obat'] .'">
        </div><br>
